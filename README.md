...
| `MAX_SCALE` | no | Mirrors the ksvc `max-scale` annotation. Rate limits count in memory per process, so they are only correct at one replica; a missing or `>1` value logs a warning at startup. |
